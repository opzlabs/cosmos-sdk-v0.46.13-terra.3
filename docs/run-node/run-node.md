...
Now that the application is ready and the keyring populated, it's time to see how to run the blockchain node. In this section, the application we are running is called [`simapp`](https://github.com/opzlabs/cosmos-sdk-v0.46.13-terra.3/tree/main/simapp), and its corresponding CLI binary `simd`. {synopsis}
...
Recall that `$MY_VALIDATOR_ADDRESS` is a variable that holds the address of the `my_validator` key in the [keyring](./keyring.md#adding-keys-to-the-keyring). Also note that the tokens in the Cosmos SDK have the `{amount}{denom}` format: `amount` is is a 18-digit-precision decimal number, and `denom` is the unique token identifier with its denomination key (e.g. `atom` or `uatom`). Here, we are granting `stake` tokens, as `stake` is the token identifier used for staking in [`simapp`](https://github.com/opzlabs/cosmos-sdk-v0.46.13-terra.3/tree/main/simapp). For your own chain with its own staking denom, that token identifier should be used instead.
...
The naive way would be to run the same commands again in separate terminal windows. This is possible, however in the Cosmos SDK, we leverage the power of [Docker Compose](https://docs.docker.com/compose/) to run a localnet. If you need inspiration on how to set up your own localnet with Docker Compose, you can have a look at the Cosmos SDK's [`docker-compose.yml`](https://github.com/opzlabs/cosmos-sdk-v0.46.13-terra.3/blob/v0.46.0-rc1/docker-compose.yml).
...
State sync works thanks to snapshots. Read how the SDK handles snapshots [here](https://github.com/opzlabs/cosmos-sdk-v0.46.13-terra.3/blob/825245d/store/snapshots/README.md).
...
```go
import (
  "github.com/opzlabs/cosmos-sdk-v0.46.13-terra.3/client/snapshot"
)
