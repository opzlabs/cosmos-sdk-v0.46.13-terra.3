...
+++ https://github.com/opzlabs/cosmos-sdk/blob/v0.46.0-rc1/types/events.go#L17-L25
...
+++ https://github.com/opzlabs/cosmos-sdk/blob/v0.46.0-rc1/types/events.go#L50-L59
...
+++ https://github.com/opzlabs/cosmos-sdk/blob/v0.46.0-rc1/x/group/keeper/msg_server.go#L89-L92
...
The `type` and `attribute` value of the `query` allow you to filter the specific Event you are looking for. For example, a `Mint` transaction triggers an Event of type `EventMint` and has an `Id` and an `Owner` as `attributes` (as defined in the [`events.proto` file of the `NFT` module](https://github.com/opzlabs/cosmos-sdk/blob/v0.46.0-rc1/proto/cosmos/nft/v1beta1/event.proto#L14-L19)).
