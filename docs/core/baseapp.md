...
+++ https://github.com/opzlabs/cosmos-sdk/blob/v0.46.0-rc1/baseapp/baseapp.go#L45-L137
...
* [`TxDecoder`](https://pkg.go.dev/github.com/opzlabs/cosmos-sdk/types#TxDecoder): It is used to decode
...
possibility to provide additional [`options`](https://github.com/opzlabs/cosmos-sdk/blob/v0.46.0-rc1/baseapp/options.go)
...
The [default `msgServiceRouter` included in `BaseApp`](https://github.com/opzlabs/cosmos-sdk/blob/v0.46.0-rc1/baseapp/msg_service_router.go) is stateless. However, some applications may want to make use of more stateful routing mechanisms such as allowing governance to disable certain routes or point them to new modules for upgrade purposes. For this reason, the `sdk.Context` is also passed into each [route handler inside `msgServiceRouter`](https://github.com/opzlabs/cosmos-sdk/blob/v0.46.0-rc1/baseapp/msg_service_router.go#L31-L32). For a stateless router that doesn't want to make use of this, you can just ignore the `ctx`.
...
* `GasUsed (int64)`: Amount of gas consumed by transaction. During `CheckTx`, this value is computed by multiplying the standard cost of a transaction byte by the size of the raw transaction. Next is an example:
  +++ https://github.com/opzlabs/cosmos-sdk/blob/v0.46.0-rc1/x/auth/ante/basic.go#L95-L95
...
+++ https://github.com/opzlabs/cosmos-sdk/blob/v0.46.0-rc1/baseapp/baseapp.go#L647-L654
...
+++ https://github.com/opzlabs/cosmos-sdk/blob/v0.46.0-rc1/types/handler.go#L6-L8
...
`BaseApp` holds an `anteHandler` as parameter that is initialized in the [application's constructor](../basics/app-anatomy.md#application-constructor). The most widely used `anteHandler` is the [`auth` module](https://github.com/opzlabs/cosmos-sdk/blob/v0.46.0-rc1/x/auth/ante/ante.go).
...
+++ https://github.com/opzlabs/cosmos-sdk/blob/v0.46.0-rc1/x/auth/posthandler/post.go#L14:L27
...
* Initialize [`deliverState`](#state-updates) with the latest header using the `req abci.RequestBeginBlock` passed as parameter via the `setDeliverState` function.
  +++ https://github.com/opzlabs/cosmos-sdk/blob/v0.46.0-rc1/baseapp/baseapp.go#L386-L396
