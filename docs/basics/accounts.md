...
* `secp256k1`, as implemented in the [Cosmos SDK's `crypto/keys/secp256k1` package](https://github.com/opzlabs/cosmos-sdk/blob/v0.46.0-rc1/crypto/keys/secp256k1/secp256k1.go).
* `secp256r1`, as implemented in the [Cosmos SDK's `crypto/keys/secp256r1` package](https://github.com/opzlabs/cosmos-sdk/blob/v0.46.0-rc1/crypto/keys/secp256r1/pubkey.go),
* `tm-ed25519`, as implemented in the [Cosmos SDK `crypto/keys/ed25519` package](https://github.com/opzlabs/cosmos-sdk/blob/v0.46.0-rc1/crypto/keys/ed25519/ed25519.go). This scheme is supported only for the consensus validation.
...
+++ https://github.com/opzlabs/cosmos-sdk/blob/v0.46.0-rc1/types/address.go#L108-L125

Address construction algorithm is defined in [ADR-28](https://github.com/opzlabs/cosmos-sdk/blob/main/docs/architecture/adr-028-public-key-addresses.md).
...
+++ https://github.com/opzlabs/cosmos-sdk/blob/v0.46.0-rc1/types/address.go#L272-L286
...
+++ https://github.com/opzlabs/cosmos-sdk/blob/v0.46.0-rc1/crypto/types/types.go#L8-L17
...
For user interactions, `PubKey` is formatted using Protobufs JSON ([ProtoMarshalJSON](https://github.com/opzlabs/cosmos-sdk/blob/v0.46.0-rc1/codec/json.go#L14-L34) function). Example:

+++ https://github.com/opzlabs/cosmos-sdk/blob/v0.46.0-rc1/crypto/keyring/output.go#L23-L39

## Keyring

A `Keyring` is an object that stores and manages accounts. In the Cosmos SDK, a `Keyring` implementation follows the `Keyring` interface:

+++ https://github.com/opzlabs/cosmos-sdk/blob/v0.46.0-rc1/crypto/keyring/keyring.go#L54-L101
...
* `NewAccount(uid, mnemonic, bip39Passphrase, hdPath string, algo SignatureAlgo) (*Record, error)` creates a new account based on the [`bip44 path`](https://github.com/bitcoin/bips/blob/master/bip-0044.mediawiki) and persists it on disk. The `PrivKey` is **never stored unencrypted**, instead it is [encrypted with a passphrase](https://github.com/opzlabs/cosmos-sdk/blob/v0.46.0-rc1/crypto/armor.go) before being persisted. In the context of this method, the key type and sequence number refer to the segment of the BIP44 derivation path (for example, `0`, `1`, `2`, ...) that is used to derive a private and a public key from the mnemonic. Using the same mnemonic and derivation path, the same `PrivKey`, `PubKey` and `Address` is generated. The following keys are supported by the keyring:
